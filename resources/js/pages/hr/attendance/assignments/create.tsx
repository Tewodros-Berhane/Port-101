...
export default function HrShiftAssignmentCreate({ employeeOptions, shiftOptions, form: defaults }: Props) {
    ...
    return (
        <AppLayout breadcrumbs={[{ title: 'Company', href: '/company/dashboard' }, { title: 'HR', href: '/company/hr' }, { title: 'Attendance', href: '/company/hr/attendance' }, { title: 'New Assignment', href: '/company/hr/attendance/assignments/create' }]}>
            <Head title="New shift assignment" />
            <div className="max-w-2xl space-y-6">
                <div className="flex items-center justify-between gap-3"><div><h1 className="text-xl font-semibold">New shift assignment</h1><p className="text-sm text-muted-foreground">Assign an employee to a defined attendance shift.</p></div><Button variant="outline" asChild><Link href="/company/hr/attendance">Back to attendance</Link></Button></div>
