...
export default function HrShiftAssignmentEdit({ employeeOptions, shiftOptions, assignment }: Props) {
    ...
    return (
        <AppLayout breadcrumbs={companyModuleBreadcrumbs(companyModuleLinks.hr, { title: 'Attendance', href: '/company/hr/attendance' }, { title: 'Edit Assignment', href: `/company/hr/attendance/assignments/${assignment.id}/edit` })}>
            <Head title="Edit shift assignment" />
            <div className="max-w-2xl space-y-6">
                <div className="flex items-center justify-between gap-3"><div><h1 className="text-xl font-semibold">Edit shift assignment</h1><p className="text-sm text-muted-foreground">Update employee attendance assignment coverage.</p></div><BackLinkAction href="/company/hr/attendance" label="Back to attendance" variant="outline" /></div>
