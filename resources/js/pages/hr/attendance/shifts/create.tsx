...
export default function HrShiftCreate({ form: defaults }: Props) {
    ...
    return (
        <AppLayout breadcrumbs={[{ title: 'Company', href: '/company/dashboard' }, { title: 'HR', href: '/company/hr' }, { title: 'Attendance', href: '/company/hr/attendance' }, { title: 'New Shift', href: '/company/hr/attendance/shifts/create' }]}>
            <Head title="New shift" />
            <div className="max-w-2xl space-y-6">
                <div className="flex items-center justify-between gap-3"><div><h1 className="text-xl font-semibold">New shift</h1><p className="text-sm text-muted-foreground">Define the expected shift window for attendance tracking.</p></div><Button variant="outline" asChild><Link href="/company/hr/attendance">Back to attendance</Link></Button></div>
