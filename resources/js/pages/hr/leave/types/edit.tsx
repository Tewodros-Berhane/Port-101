...
export default function HrLeaveTypeEdit({ units, leaveType }: Props) {
    ...
    return (
        <AppLayout breadcrumbs={[{ title: 'HR', href: '/company/hr' }, { title: 'Leave', href: '/company/hr/leave' }, { title: 'Edit type', href: `/company/hr/leave/types/${leaveType.id}/edit` }]}>
            <Head title="Edit leave type" />
            <div className="flex items-center justify-between gap-3"><div><h1 className="text-xl font-semibold">Edit leave type</h1><p className="text-sm text-muted-foreground">Adjust allocation, approval, and balance settings for this leave type.</p></div><Button variant="ghost" asChild><Link href="/company/hr/leave">Back</Link></Button></div>
