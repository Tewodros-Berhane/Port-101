...
type Props = { employeeOptions: EmployeeOption[]; leaveTypes: LeaveTypeOption[]; leavePeriods: LeavePeriodOption[]; allocation: { id: string; employee_id: string; leave_type_id: string; leave_period_id: string; allocated_amount: string; used_amount: string; carry_forward_amount: string; expires_at: string; notes: string; } };

export default function HrLeaveAllocationEdit({ employeeOptions, leaveTypes, leavePeriods, allocation }: Props) {
    const form = useForm(allocation);

    return (
        <AppLayout breadcrumbs={moduleBreadcrumbs(companyModuleLinks.hr, { title: 'Leave', href: '/company/hr/leave' }, { title: 'Edit allocation', href: `/company/hr/leave/allocations/${allocation.id}/edit` })}>
            <Head title="Edit leave allocation" />
            <div className="flex items-center justify-between gap-3"><div><h1 className="text-xl font-semibold">Edit leave allocation</h1><p className="text-sm text-muted-foreground">Adjust entitlement and carry-forward values for this employee.</p></div><BackLinkAction href="/company/hr/leave" label="Back to leave" variant="ghost" /></div>
            <form className="mt-6 grid gap-6" onSubmit={(event) => { event.preventDefault(); form.put(`/company/hr/leave/allocations/${allocation.id}`); }}>
                <div className="grid gap-6 md:grid-cols-2">
                    <Field label="Employee" error={form.errors.employee_id}><select className="h-9 rounded-md border border-input bg-transparent px-3 py-1 text-sm" value={form.data.employee_id} onChange={(event) => form.setData('employee_id', event.target.value)} required>{employeeOptions.map((employee) => <option key={employee.id} value={employee.id}>{employee.name}{employee.employee_number ? ` (${employee.employee_number})` : ''}</option>)}</select></Field>
                    <Field label="Leave type" error={form.errors.leave_type_id}><select className="h-9 rounded-md border border-input bg-transparent px-3 py-1 text-sm" value={form.data.leave_type_id} onChange={(event) => form.setData('leave_type_id', event.target.value)} required>{leaveTypes.map((leaveType) => <option key={leaveType.id} value={leaveType.id}>{leaveType.name} ({leaveType.unit})</option>)}</select></Field>
                    <Field label="Leave period" error={form.errors.leave_period_id}><select className="h-9 rounded-md border border-input bg-transparent px-3 py-1 text-sm" value={form.data.leave_period_id} onChange={(event) => form.setData('leave_period_id', event.target.value)} required>{leavePeriods.map((leavePeriod) => <option key={leavePeriod.id} value={leavePeriod.id}>{leavePeriod.name}</option>)}</select></Field>
                ...
                <div><Button type="submit" disabled={form.processing}>Update allocation</Button></div>
            </form>
        </AppLayout>
    );
}
