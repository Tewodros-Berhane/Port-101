...
- Master data CRUD: controllers, requests, Inertia pages, and routes for partners, contacts, products, taxes, currencies, uoms, price lists.
- Permissions: per-entity permissions seeded; owner role gets all permissions; legacy core.master_data.manage removed.
- Authorization wiring: policies for Partner/Product/Tax/Currency/Uom/PriceList; AuthServiceProvider registers them; controllers call `$this->authorize()` for CRUD actions; base Controller uses AuthorizesRequests.
...
- Addresses CRUD (migrations exist, no controllers/pages/routes yet).
