...
- Latest full-suite count: `311 passed`, `0 failed`.
- Result summary after latest implementation: HR employee onboarding now supports employee-driven system-access provisioning with role selection, pending invite issuance, resend/cancel/deactivate/reactivate actions, active-role changes, existing-user linking, and invite-acceptance linkage back into `hr_employees`, while `/company/users` is now a read-only active-user directory and the PostgreSQL-backed suite remains fully passing across the broader platform, module, API, integration, and operational-hardening coverage.
